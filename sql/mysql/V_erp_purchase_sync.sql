...
CREATE TABLE `erp_purchase_order` (
  ...
  `amount_total` decimal(12,2) DEFAULT NULL COMMENT '应付货款（实际入库量计算后）',
  `quantity_real` int DEFAULT NULL COMMENT '实际采购量（整单）',
  `quantity_entry` int DEFAULT NULL COMMENT '入库量（整单）',
...
CREATE TABLE `erp_purchase_order_item` (
  ...
  `quantity_entry` int DEFAULT NULL COMMENT '到货入库量',
  `base_price` decimal(14,4) DEFAULT NULL COMMENT '基准单价（根据基准单价N配置规则计算）',
  `item_status` tinyint NOT NULL DEFAULT '1' COMMENT '采购有效状态：0-待审核 1-有效 2-无效',
