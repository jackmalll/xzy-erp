package cn.xzy.module.iot.core.messagebus.config;

import cn.xzy.framework.mq.redis.core.RedisMQTemplate;
import cn.xzy.framework.mq.redis.core.job.RedisPendingMessageResendJob;
import cn.xzy.framework.mq.redis.core.job.RedisStreamMessageCleanupJob;
import cn.xzy.framework.mq.redis.core.stream.AbstractRedisStreamMessage;
import cn.xzy.framework.mq.redis.core.stream.AbstractRedisStreamMessageListener;
import cn.xzy.module.iot.core.messagebus.core.IotMessageBus;
import cn.xzy.module.iot.core.messagebus.core.local.IotLocalMessageBus;
import cn.xzy.module.iot.core.messagebus.core.redis.IotRedisMessageBus;
import cn.xzy.module.iot.core.messagebus.core.rocketmq.IotRocketMQMessageBus;
...
public class IotMessageBusAutoConfiguration {
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "xzy.iot.message-bus", name = "type", havingValue = "local", matchIfMissing = true)
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "xzy.iot.message-bus", name = "type", havingValue = "rocketmq")
    ...
    /**
     * 特殊：由于 XzyRedisMQConsumerAutoConfiguration 关于 Redis stream 的消费是动态注册，所以这里只能拷贝相关的逻辑！！！
     *
     * @see cn.xzy.framework.mq.redis.config.XzyRedisMQConsumerAutoConfiguration
     */
    @Configuration
    @ConditionalOnProperty(prefix = "xzy.iot.message-bus", name = "type", havingValue = "redis")
